...
def get_max_oversampling_and_min_n_steps(volume, n_domains,
    ...
    """
    Returns
        max_oversampling, min_n_steps
    Given volume, n_domains and target values for coverage and selection probability.
    max_oversampling is found so that:
        max_selection_prob = max_oversampling / (N_AVOGADRO*volume)
    I've found empirically that max_selection_prob should be no higher than 1e-4.
    If it is, e.g. 1e-3 or 1e-2, then you will get weird shoulder just above T=Tm.
    Then, N_steps is selected so that:
        target_coverage = min_n_steps*oversampling/(n_domains*N_AVOGADRO*volume)
    If target_coverage is <<1, then the system will not have enough time (steps)
    to reach equilibrum.
    """
