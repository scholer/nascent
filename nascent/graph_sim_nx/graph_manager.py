...
from .looptracking import (
group_interfaces_path_by_stiffness,
interfaces_shortest_path,
interfaces_path_segments,
grouped_path_edges_to_segments,
join_two_edge_groups,
join_edges_groups,
closed_loop_segments,
calculate_loop_activity,
loop_formation_activity,
loop_breakage_effects,
find_alternative_shortest_path,
loop_formation_effects,
find_maybe_shorter_e1_e3_subpaths,
...
class GraphManager(object):
    ...
    def interfaces_shortest_path(self, ifnode1, ifnode2):
        """ Now just a simple wrapper around looptracking.interfaces_shortest_path """
        return interfaces_shortest_path(self.interface_graph, ifnode1, ifnode2)

    def group_interfaces_path_by_stiffness(self, path):
        """ Now just a simple wrapper around looptracking.group_interfaces_path_by_stiffness """
        return group_interfaces_path_by_stiffness(self.interface_graph, path)

    def interfaces_path_segments(self, path):
        """ Now just a simple wrapper around looptracking.interfaces_path_segments """
        return interfaces_path_segments(self.interface_graph, path)

    def grouped_path_edges_to_segments(self, path):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return grouped_path_edges_to_segments(path)

    def join_two_edge_groups(self, part_a, part_b, simulate_reaction, reaction_elems=(0, None)):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return join_two_edge_groups(self.interface_graph, part_a, part_b, simulate_reaction, reaction_elems)

    def join_edges_groups(self, edge_groups, simulate_reaction=None, reaction_elems=(0, None)):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return join_edges_groups(self.interface_graph, edge_groups, simulate_reaction, reaction_elems)

    def closed_loop_segments(self, loop_path, simulate_reaction=None, reaction_elems=(0, None)):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return closed_loop_segments(self.interface_graph, loop_path, simulate_reaction, reaction_elems)

    def calculate_loop_activity(self, loop_path, simulate_reaction=None, reaction_elems=(0, None), circularize_path=False):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return calculate_loop_activity(self.interface_graph, loop_path, simulate_reaction, reaction_elems, circularize_path)

    def loop_formation_activity(self, elem1, elem2, reaction_type):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return loop_formation_activity(self.interface_graph, elem1, elem2, reaction_type)

    def loop_breakage_effects(self, elem1, elem2, reaction_type):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return loop_breakage_effects(self.interface_graph, elem1, elem2, reaction_type)

    def loop_formation_effects(self, elem1, elem2, reaction_type):
        """ Now just a simple wrapper around looptracking.grouped_path_edges_to_segments """
        return loop_formation_effects(self.interface_graph, elem1, elem2, reaction_type)
