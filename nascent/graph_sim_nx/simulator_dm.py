    ...
    def simulate(self, T=None, n_steps_max=100000, simulation_time=None, systime_max=None):
        ...
        while n_done < n_steps_max:
            ...
            if hyb_result['changed_complexes']:
                changed_domains += [domain for cmplx in hyb_result['changed_complexes'] for domain in cmplx.nodes()
                                    if domain.partner is None and domain.name in sysmgr.domain_pairs]
            if hyb_result['new_complexes']:
                changed_domains += [domain for cmplx in hyb_result['new_complexes'] for domain in cmplx.nodes()
                                    if domain.partner is None and domain.name in sysmgr.domain_pairs]
            if hyb_result['free_strands']:
                changed_domains += [domain for strand in hyb_result['free_strands'] for domain in strand.domains
                                    if domain.partner is None and domain.name in sysmgr.domain_pairs]
            if is_hybridizing:
            ...
            if len(changed_domains) != len(set(changed_domains)):
                ...
                print("changed_complexes:")
                pprint(changed_complexes)
                print("changed_complexes domains: (using complex.nodes())")
                pprint([domain for cmplx in changed_complexes for domain in cmplx.nodes()])
                print("changed_complexes domains: (using complex.strands)")
                pprint([domain for cmplx in changed_complexes for s in cmplx.strands for domain in s.domains])
                print("free_strands:")
                pprint(free_strands)
                print("free_strands domains:")
                pprint([d for s in free_strands for d in s.domains])
